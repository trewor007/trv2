...
def KonektorWebsocketSubskribe():
    ...
    while x < 1000:
        ...
        c.execute("INSERT INTO tabelka2(type, side, price, time, order_id, product_id, order_type, size, reason, remaining_size, client_oid, sequence) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (typtranzakcji, kupsprzedaj, cena, czas, orderid, parawalut, typzamowienia, rozmiar, powod, rozmiar2, klientoid, sekwencja))
        x=x+1 
    conn.commit()
...
ws.send(json.dumps({'type': 'subscribe', 'product_ids': produkty, 'channels': kanaly}))    #wysłanie subskrybcji
a=time.time()
if arg1==0: 
