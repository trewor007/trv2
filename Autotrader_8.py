...
class Websocket(object):
    ...
    def __init__(self, wsurl="wss://ws-feed.gdax.com", dane=None, ws=None, kanaly=None, ping_start=0,produkty=['ETH-EUR', 'LTC-EUR', 'BTC-EUR'], newdict={}):
        self.wsurl = wsurl
        self.ws = None
        self.dane=dane
...
class Requester(object):
    ...
    def Historic_rates(self, start=None, end=None, skala=None):
        parametry={}
        skala=300
        end= datetime.datetime.fromtimestamp(1524939014.1383)
        print(end)
        start=datetime.datetime.fromtimestamp(1524900614.1383)
        print(start)
        ...
        print(self.produkty[0])
        return self._get('/products/{}/candles'.format(str(self.produkty[0])), params=parametry)
...
class Autotrader():
    ...
    a=int(input("[1 BTC-EUR] [2 LTC-EUR] [3 ETH-EUR] "))
    if a==1:
        produkty=["BTC-EUR"]
    elif a==2:
        produkty=["LTC-EUR"]
    ...
    if b=='1':
        ...
        webs=Websocket(produkty=produkty, kanaly=kanaly)
        webs.Polacz()        
    elif b=='2':
        webr=Requester(produkty=produkty)
        webr.Historic_rates() 
    print(b)      
