...
while True:
    if q.not_empty:            
        ...
        if typ=='ticker':
            price=float(dane.get('price', None))
    ...
    if b==True:
        ...
        if len(cena[produkt_id])==0:
            ...
            #Ru=Rules()
            #Ru.SMA_EMA(produkt_id=produkt_id, zakres=zakres, cena=cena[produkt_id][4], smas_budget=smas_budget[produkt_id], smas_budget2=smas_budget2[produkt_id], ema_budget=ema_budget[produkt_id], ema_budget2=ema_budget2[produkt_id], smas=smas[produkt_id], rsi=rsi[produkt_id], ema=ema[produkt_id], wallet=wallet)
            #for x in range(len(produkty)):
